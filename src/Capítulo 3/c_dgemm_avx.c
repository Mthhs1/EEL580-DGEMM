...
/*
Primeira otimizacao em C baseada no capitulo 3.8 (subword parallelism):
- Vetoriza 4 elementos de C por iteracao no eixo i usando registradores AVX de 256 bits.
- Usa broadcast do elemento de B e FMA para acumular A*B em paralelo.
- Segue a estrutura do livro com 3 loops e, por isso, assume n multiplo de 4.
*/
static void dgemm_c_avx(size_t n, const double *A, const double *B, double *C)
{
...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        printf("Uso: ./c_dgemm_avx <tamanho_da_matriz>\n");
        return 1;
    }
    ...
    if (n % 4 != 0)
    {
        printf("Para manter a estrutura AVX igual a do livro, informe um tamanho multiplo de 4.\n");
        return 1;
    }
    ...
    snprintf(log_path, sizeof(log_path), "c_dgemm_avx_log_%zu.txt", n);
    ...
    log_message(log_file, "Executando %d testes de DGEMM em C (Otimizacao 1 - AVX)", NUM_TESTS);
    ...
    log_message(log_file, "Implementacao: intrinsics AVX (_mm256_loadu_pd, _mm256_broadcast_sd, _mm256_fmadd_pd)");
...
gcc -O3 -mavx2 -mfma -o c_dgemm_avx c_dgemm_avx.c -lm
